...
def choose_experiment_mode(df, numeric_columns):
    ...
    if option == '1':
        return df.copy(), None, option
    elif option == '2':
        df_clean = df[df[numeric_columns].notna().all(axis=1)].reset_index(drop=True)
...
def knn_imputation_optuna(X_numeric, df_original, df_before_removal, numeric_columns, timeout):
    print("Optimizing n_neighbors for KNNImputer using Optuna...")
    ...
    def objective(trial):
        ...
        X_imputed = knn_imputer.fit_transform(X_numeric)
    ...
    return KNNImputer(n_neighbors=best_n).fit_transform(X_numeric)

def svd_imputation_optuna(X_numeric, df_original, df_before_removal, numeric_columns, timeout):
    print("Optimizing IterativeSVD with Optuna...")
    true_values = df_before_removal[numeric_columns].to_numpy()

    def objective(trial):
        rank = trial.suggest_int("rank", 1, min(X_numeric.shape) - 1)
        max_iters = trial.suggest_int("max_iters", 100, 1000)
        threshold = trial.suggest_float("convergence_threshold", 1e-5, 1e-2, log=True)

        model = IterativeSVD(rank=rank, max_iters=max_iters, convergence_threshold=threshold)
        X_imputed = model.fit_transform(X_numeric)
...
def impute_class_variable(df_imputed, X):
    ...
    if class_imputation == '4':
        ...
        knn.fit(X[mask_not_null], df_temp.loc[mask_not_null, target_column].astype(int))
        preds = knn.predict(X[~mask_not_null])
...
def model_selection(catalogs, numeric_columns, timeout=600):
    df = catalogs['cat_fortin'].copy()
    df_original, df_before_removal, option = choose_experiment_mode(df, numeric_columns)
...
df_highlighted = model_selection({'cat_fortin': df_base}, numeric_cols_to_use, timeout=30)
...
if df_highlighted is not None:
    ...
    print("Imputation could not be completed.")
#%%
